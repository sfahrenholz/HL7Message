...
public class XTN extends Datatype {
	...
	/**
	 * nternationalization note:  To make this data type interoperate with CEN’s Telecommunication data attribute group, we allow use of the second component for email addresses. The presence of an email address is specified by the addition of the value NET  to the Phone Use Code table, and the type of Internet address is specified with the values Internet  and X.400  to the Phone Equipment Type table. When used for an Internet address, the first component of the XTN data type will be null. If the @-sign is being used as a subcomponent delimiter, the HL7 subcomponent escape sequence may be used when encoding an Internet address (see HL7 International v2.4 Section 2.10.1, “Formatting codes”).
	 * @return
	 */
